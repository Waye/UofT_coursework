...
/**
 * This is a image class. Image are created when read new image Images can be stored after
 * application closed. Images can be renamed, add tag, delete tag, image name will change follow
 * with different tags.
 */
public class Images extends ImageInfo implements Serializable {
  ...
  public Images(String directory) throws IOException {
    ...
    // this.index1 = finder.images.indexOf(new Images(directory));
    ...
    for (String tag : tagsInName()) {
      if (!Tags.availableTags.contains(tag)) {
   ...
   * @throws IOException when tag cannot be saved to serialized file or deleted from file name
   *     properly
   */
  public void deleteTag(String tag) throws IOException {
    ...
    if (success) {
      ...
      usetags.delTolist(Tags.imageMappingTag, this, tag);
      Logging.recordLogInfo(oldName, newName);
    }

    // tagss.delTolist(Tags.imageMappingTag, tag, this.getName());
  }
  ...
  public void addTag(String tag) throws IOException {
    ...
    // check if key exist
    // tagss.addToList(Tags.imageMappingTag, this.getName(), tag);

    List<String> temp = new ArrayList<>();
    temp.add(tag);
    usetags.addToList(Tags.imageMappingTag, this, tag);
